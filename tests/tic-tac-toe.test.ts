...
function cancelGame(user: string) {
  return simnet.callPublicFn(
    "tic-tac-toe",
    "cancel-game",
    [Cl.uint(0)],
...
describe("Tic Tac Toe Tests", () => {
  ...
  it("allows player two to win", () => {
    ...
    expect(gameData).toBeSome(
      Cl.tuple({
        ...
        "last-move-time": Cl.uint(8),
        cancelled: Cl.bool(false),
      })
    );
  });

  it("does not allow cancelling a game before timeout", () => {
    createGame(100, 0, 1, alice);
    joinGame(1, 2, bob);

    const { result } = cancelGame(bob);
    expect(result).toBeErr(Cl.uint(106));
  });

  it("allows cancelling a game after timeout", () => {
    createGame(100, 0, 1, alice);
    joinGame(1, 2, bob);

    // Advance blocks to exceed threshold
    simnet.mineEmptyBlocks(145);

    const { result, events } = cancelGame(bob); // bob is the waiting player
    expect(result).toBeOk(Cl.bool(true));
    expect(events.length).toBe(3); // print_event and 2 stx_transfer_events
    ...
    expect(gameData).toBeSome(
      Cl.tuple({
        ...
        winner: Cl.none(),
        "last-move-time": Cl.uint(4),
        cancelled: Cl.bool(true),
      })
    );
  });

  it("does not allow cancelling by the wrong player", () => {
    createGame(100, 0, 1, alice);
    joinGame(1, 2, bob);

    simnet.mineEmptyBlocks(145);

    const { result } = cancelGame(alice); // alice is not the waiting player
    expect(result).toBeErr(Cl.uint(104));
  });

  it("does not allow cancelling a cancelled game", () => {
    createGame(100, 0, 1, alice);
    joinGame(1, 2, bob);
  ...
  it("does not allow cancelling a game with only one player", () => {
    createGame(100, 0, 1, alice);

    simnet.mineEmptyBlocks(145);

    const { result } = cancelGame(alice);
    expect(result).toBeErr(Cl.uint(103));
  });
});
