...
export async function getGame(gameId: number) {
  ...
  const game: Game = {
    ...
    "last-move-time": gameCV["last-move-time"] ? parseInt(gameCV["last-move-time"].value.toString()) : 0,
    cancelled: gameCV["cancelled"] ? cvToValue(gameCV["cancelled"]) : false,
  };
  return game;
}
