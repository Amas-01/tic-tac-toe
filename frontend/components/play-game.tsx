...
export function PlayGame({ game }: PlayGameProps) {
  const { userData, handleJoinGame, handlePlayGame, handleCancelGame } = useStacks();
  ...
  const isWaitingPlayer =
    isJoinedAlready && !isMyTurn && !isGameOver && !game.cancelled;
  ...
  return (
    <div className="flex flex-col gap-4 w-[400px]">
      ...
      <div className="flex flex-col gap-2">
        ...
        {isCancelled && (
          <div className="text-red-500">Game Cancelled</div>
        )}
      </div>
      ...
      {isWaitingPlayer && (
        ...
        >
          Cancel Game (if opponent inactive)
        </button>
      )}

      {isJoinedAlready && !isMyTurn && !isGameOver && !isWaitingPlayer && (
