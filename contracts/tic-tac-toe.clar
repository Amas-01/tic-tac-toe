...
(define-constant ERR_GAME_CANCELLED u105) ;; Error thrown when trying to act on a cancelled game
(define-constant ERR_TIMEOUT_NOT_REACHED u106) ;; Error thrown when trying to cancel a game before timeout
(define-constant THRESHOLD_BLOCKS u144) ;; ~24 hours in blocks (144 * 10 min = 1440 min = 24 hours)
...
(define-public (create-game (bet-amount uint) (move-index uint) (move uint))
    (let (
        ...
        (game-data {
            ...
            board: game-board,
            winner: none,
            last-move-time: stacks-block-height,
            cancelled: false
        })
    )
...
(define-public (play (game-id uint) (move-index uint) (move uint))
    ...
    ;; Ensure that the game has not been cancelled
    (asserts! (not (get cancelled original-game-data)) (err ERR_GAME_CANCELLED))
...
(define-public (cancel-game (game-id uint))
    (let (
        ...
        (waiting-player (if is-player-one-turn (unwrap! (get player-two game-data) (err ERR_GAME_NOT_FOUND)) (get player-one game-data)))
        (bet-amount (get bet-amount game-data))
        (last-move-time (get last-move-time game-data))
        (current-height stacks-block-height)
    )
    ;; Ensure the game is not already cancelled
    (asserts! (not (get cancelled game-data)) (err ERR_GAME_CANCELLED))
    ;; Ensure both players have joined
    (asserts! (is-some (get player-two game-data)) (err ERR_GAME_CANNOT_BE_JOINED))
    ;; Ensure the caller is the waiting player
    (asserts! (is-eq contract-caller waiting-player) (err ERR_NOT_YOUR_TURN))
    ;; Ensure enough time has passed
    (asserts! (> current-height (+ last-move-time THRESHOLD_BLOCKS)) (err ERR_TIMEOUT_NOT_REACHED))

    ;; Transfer funds back to players
    (try! (as-contract (stx-transfer? bet-amount tx-sender (get player-one game-data))))
    (try! (as-contract (stx-transfer? bet-amount tx-sender (unwrap! (get player-two game-data) (err ERR_GAME_NOT_FOUND)))))

    ;; Mark game as cancelled
    (map-set games game-id (merge game-data { cancelled: true }))

    ;; Log the cancellation
    (print { action: "cancel-game", game-id: game-id })
    (ok true)
))
